...
%     end
    ...
    if a_flag
        rows=length(temp.textdata(:,1))-1; %# of rows - header
        ...
        for i=1:length(temp.textdata(1,:)) %for all columns on row 1
            ...
            else %enter corresponding data into new substrempuct
             %   temp2.(sprintf('%s',str))= [];
                if i-2 > length(temp.data(1,:)); %if temp.data ends before temp.textdata
                    
                    
                    
                    temp2(:).(sprintf('%s',str))= NaN(rows,1);
